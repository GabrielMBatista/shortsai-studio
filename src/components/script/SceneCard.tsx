...
const SceneCard: React.FC<SceneCardProps> = ({ scene, sceneIndex, onRegenerateImage, onRegenerateAudio, onUpdateScene }) => {
    ...
    const handleRegenClick = (type: 'image' | 'audio') => {
        ...
        const force = isCompleted; 
        
        if (force) {
             if (!window.confirm(`Regenerate ${type}? This will overwrite the existing file.`)) return;
        }
              ...
              <div className="flex-1 min-h-0 flex flex-col gap-2">
                ...
                <div className="relative">
                    <textarea 
                        ref={textareaRef}
                        readOnly={!isEditing}
                        className={`w-full rounded-lg p-3 text-sm text-slate-200 leading-relaxed resize-none transition-all min-h-[80px] ${
                            isEditing 
