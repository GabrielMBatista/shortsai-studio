...
import { Sparkles, ArrowRight, Mic, Play, Square, Loader2, ChevronDown, Music, Zap, Image as ImageIcon, LayoutTemplate, Plus, User as UserIcon, Trash2, CheckCircle2, X, Upload, Palette, Film, Paintbrush, Box, MonitorPlay, Wand2 } from 'lucide-react';
import { generateNarrationAudio, analyzeCharacterFeatures } from '../services/geminiService';
    ...
    const handlePlay = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            if (!url && provider === 'gemini') {
                url = await generateNarrationAudio(`Hello! I am ${vObj.label}.`, vObj.name);
            }
...
interface InputSectionProps {
  user: User | null;
  onGenerate: (topic: string, style: string, voice: string, provider: TTSProvider, language: string, refs: any[], opt: boolean, music: boolean) => void;
  isLoading: boolean;
...
const InputSection: React.FC<InputSectionProps> = ({ user, onGenerate, isLoading, loadingMessage, showToast }) => {
  ...
  useEffect(() => {
    ...
    const fetchVoices = async () => {
        if (ttsProvider === 'elevenlabs') {
            const v = await getElevenLabsVoices();
            setDynamicVoices(v);
            if (v.length > 0) {
                // If current voice is not in list (or empty), select first
  ...
  const handleSaveChar = async () => {
      try {
          const char = await addCharacter(newCharName, newCharDesc, newCharImages);
          if (char) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
      ...
      onGenerate(topic, style, voice, ttsProvider, language, selectedRefs, optimizeRef, includeMusic && IS_SUNO_ENABLED);
  };
        ...
        <div className="lg:col-span-7 space-y-8">
            ...
            <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6 shadow-xl hover:border-slate-600 transition-colors">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                         <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 ml-1">Output Language</label>
            ...
            <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6 shadow-xl">
                    ...
                    {VIDEO_STYLES.map((s) => {
                        ...
                        return (
                            <button
                                ...
                                type="button"
                                onClick={() => setStyle(s)}
                                className={`relative group p-3 rounded-xl border text-left transition-all duration-200 h-20 flex flex-col justify-between ${
                                isSelected 
                ...
                <div className="mt-8 border-t border-slate-700/50 pt-6">
                    ...
                    {selectedCharIds.length > 0 && (
                         <div className="mt-2 bg-indigo-500/10 border border-indigo-500/20 rounded-lg p-3 flex items-center gap-3 animate-fade-in-up">
                            <input type="checkbox" checked={optimizeRef} onChange={e => setOptimizeRef(e.target.checked)} className="rounded bg-slate-900 border-slate-600 text-indigo-600 focus:ring-offset-0 focus:ring-0 w-4 h-4 cursor-pointer" />
                            <div>
                                <span className="text-xs font-bold text-indigo-200 block">AI Character Optimization</span>
                                <span className="text-[10px] text-indigo-300/70 block">Generate a neutral headshot first to improve consistency across scenes.</span>
                            </div>
                         </div>
                    )}
                </div>
        ...
        <div className="lg:col-span-5 space-y-8">
            <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6 shadow-xl h-full flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-1 bg-slate-900 p-1.5 rounded-xl mb-6">
                    <button type="button" onClick={() => setTtsProvider('gemini')} className={`py-2.5 rounded-lg text-xs font-bold transition-all ${ttsProvider === 'gemini' ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}>Google Gemini</button>
                    <button type="button" onClick={() => setTtsProvider('elevenlabs')} className={`py-2.5 rounded-lg text-xs font-bold transition-all ${ttsProvider === 'elevenlabs' ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}>ElevenLabs</button>
                </div>

                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Voice Model</label>
                <div className="flex gap-3 mb-8">
                    <div className="relative flex-1">
                        <select value={voice} onChange={(e) => setVoice(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl pl-4 pr-10 py-3 text-white appearance-none cursor-pointer hover:border-slate-500 transition-colors h-12">
                ...
                {IS_SUNO_ENABLED && (
                    <div 
                        ...
                        className={`mt-auto relative overflow-hidden p-5 rounded-2xl border cursor-pointer flex items-center gap-4 transition-all duration-300 group ${
                            includeMusic ? 'bg-gradient-to-br from-pink-500/20 to-purple-600/20 border-pink-500/50' : 'bg-slate-900 border-slate-700 hover:border-pink-500/30'
                        }`}
                    >
            ...
            >
                {isLoading ? (
                    <div className="flex items-center gap-4 py-2">
                        <Loader size="sm" /> 
                        <span className="animate-pulse">{loadingMessage || "Processing..."}</span>
                    </div>
                ) : (
                    <>
